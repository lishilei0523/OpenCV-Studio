...
namespace SD.OpenCV.OnnxRuntime.Models
{
    ...
    /// YOLO定向目标检测模型
    /// </summary>
    public class YoloObbDetector : OnnxModel<Mat, ObbDetection[]>
    {
        #region # 字段及构造器
        ...
        public YoloObbDetector(byte[] modelBytes)
        ...
        public YoloObbDetector(string modelPath, SessionOptions sessionOptions)
        ...
        /// <param name="modelBytes">模型字节数组</param>
        /// <param name="sessionOptions">会话选项</param>
        public YoloObbDetector(byte[] modelBytes, SessionOptions sessionOptions)
        ...
        #region # 处理推理结果 —— override ObbDetection[] ProcessInference(IReadOnlyList...
        /// <summary>
        /// 处理推理结果
        /// </summary>
        ...
        protected override ObbDetection[] ProcessInference(IReadOnlyList<NamedOnnxValue> inference, float minConfidence)
        {
            ...
            for (int rowIndex = 0; rowIndex < transposedMat.Rows; rowIndex++)
            {
                ...
                if (maxConfidence >= minConfidence)
                {
                    ...
                    Size2f size = new Size2f(width, height);
                    int maxConfidenceIndex = Array.IndexOf(confidencesArray, maxConfidence);
                    RotatedRect rotatedBox = new RotatedRect(center, size, degrees);

                    ObbDetection detection = new ObbDetection(labels[maxConfidenceIndex], rotatedBox, maxConfidence);
                    detections.Add(detection);
                }
            }

            //NMS
            CvDnn.NMSBoxes(detections.Select(x => x.RotatedBox), detections.Select(x => x.Confidence), ScoreThreshold, NmsThreshold, out int[] indices);
            ObbDetection[] nmsDetections = detections.Where((_, index) => indices.Contains(index)).ToArray();
        ...
        {
            string[] labels = AppDomain.CurrentDomain.GetData(typeof(YoloObbDetector).FullName!) as string[];
            if (labels == null)
            {
                labels = File.ReadAllLines("Content/Labels/yolo_obb_labels.txt");
            }
