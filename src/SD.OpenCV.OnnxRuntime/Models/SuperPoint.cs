...
namespace SD.OpenCV.OnnxRuntime.Models
{
    ...
    /// SuperPoint模型
    /// </summary>
    public class SuperPoint : OnnxModel<Mat, Feature[]>
    {
        #region # 字段及构造器
        ...
        public SuperPoint(string modelPath)
        ...
        public SuperPoint(byte[] modelBytes)
        ...
        public SuperPoint(string modelPath, SessionOptions sessionOptions)
        ...
        /// <param name="modelBytes">模型字节数组</param>
        /// <param name="sessionOptions">会话选项</param>
        public SuperPoint(byte[] modelBytes, SessionOptions sessionOptions)
        ...
        protected override unsafe List<NamedOnnxValue> ProcessInput(Mat input)
        {
            ...
            DenseTensor<float> sourceTensor = new DenseTensor<float>([1, 1, SideSize, SideSize]);//[批次数, 通道数, 图像高, 图像宽]
            resizedImage.ForEachAsByte((valuePtr, positionPtr) =>
            {
                ...
                //按模型要求标准化处理像素
                sourceTensor[0, 0, rowIndex, colIndex] = pixel / 255f;
            });
