...
namespace SD.OpenCV.OnnxRuntime.Models
{
    ...
    /// XFeat模型
    /// </summary>
    public class XFeatPoint : OnnxModel<Mat, Feature[]>
    {
        #region # 字段及构造器
        ...
        public XFeatPoint(string modelPath)
        ...
        public XFeatPoint(byte[] modelBytes)
        ...
        public XFeatPoint(string modelPath, SessionOptions sessionOptions)
        ...
        /// <param name="modelBytes">模型字节数组</param>
        /// <param name="sessionOptions">会话选项</param>
        public XFeatPoint(byte[] modelBytes, SessionOptions sessionOptions)
        ...
        {
            Tensor<float> keyPointsTensor = inference[0].AsTensor<float>();
            Tensor<float> descriptorsTensor = inference[1].AsTensor<float>();
            Tensor<float> scoresTensor = inference[2].AsTensor<float>();
            ...
            //解析推理结果
            using Mat rawKeyPoints = Mat.FromArray(keyPointsTensor);
            using Mat rawDescriptors = Mat.FromArray(descriptorsTensor);
            using Mat keyPoints = rawKeyPoints.Reshape(1, keyPointsTensor.Dimensions[0]);
            using Mat descriptors = rawDescriptors.Reshape(1, descriptorsTensor.Dimensions[0]);
            IList<Feature> features = new List<Feature>();
            for (int rowIndex = 0; rowIndex < keyPoints.Rows; rowIndex++)
            {
                float confidence = scoresTensor[rowIndex];
                if (confidence >= minConfidence)
                {
                    ...
                    rowKeyPointMat.GetArray(out float[] rowKeyPoint);
