...
namespace OpenCV.Client.ViewModels.CalibrationContext
{
    ...
    public class MonoViewModel : ScreenBase
    {
        ...
        public async void DrawKeyPoints()
        {
            ...
            bool success = await Task.Run(() => grayImage.GetOptimizedChessboardCorners(patternSize, 30, 0.01, out cornerPoints));
            if (success)
            {
        ...
        public async void ExecuteCalibration()
        {
            ...
            if (this.Images.Count < 8)
            {
                MessageBox.Show("图像数量不可小于8！", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            if (this.ParamViewModel == null)
            {
        ...
        public async void ExportResult()
        {
            ...
            {
                await Task.Run(() => File.WriteAllText(saveFileDialog.FileName, json));
            }
